...
my $cmd = "$wasprofile/bin/wsadmin.$shellExt  -lang jython -user $wasUser -password $wasPwd -f $homePath/bin/was-app-poststop.py";
$cmd = "\"$wasprofile/bin/wsadmin.$shellExt\"  -lang jython -user $wasUser -password $wasPwd -f \"$homePath/bin/was-app-poststop.py\""
    if ( $ostype eq 'windows' );
if ( not defined($wasPwd) or $wasPwd eq '' ) {
    $cmd = "$wasprofile/bin/wsadmin.$shellExt  -lang jython -f $homePath/bin/was-app-poststop.py";
    $cmd = "\"$wasprofile/bin/wsadmin.$shellExt\"  -lang jython -f \"$homePath/bin/was-app-poststop.py\"" if ( $ostype eq 'windows' );
}
