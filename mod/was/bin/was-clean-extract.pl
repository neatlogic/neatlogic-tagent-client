...
if ( $needDeploy == 1 ) {
    ...
    for ( my $i = 0 ; $i < $appNum ; $i++ ) {
        ...
        if ( $appFile =~ /\.war$/ ) {
            ...
            if ( -d $targetDir and -d $descTarget ) {
                ...
                if ( scalar(@warFiles) > 0 ) {

                    #my @targetDirWars = Utils::getDirInfo( $targetDir, 1, "war" );
                    foreach my $warFile (@warFiles) {

                        #print("INFO: Remove war $targetDir/*.war\n");
                        #system("rm -rf $targetDir/*.war");
                        print("INFO: Remove war $targetDir/*.war\n");
                        rmtree("$targetDir/$warFile");
                    }
                }
            ...
            if ( defined($ihsRoot) and $ihsRoot ne '' and ( -d $ihsRoot ) and $ihsRoot ne '/' ) {
                ...
                #my $unzipCmd = "unzip -qo $pkgsDir/$insName/$appFile -d $extractTmp";
    ...
    if ( $isFirstDeploy == 1 and -d $wasprofile ) {
        ...
        if ( not defined($wasPwd) or $wasPwd eq '' ) {
            my $deployCmd = "$wasprofile/bin/wsadmin.$shellExt  -lang jython -f $homePath/bin/was-autodeploy.py";
        }

        #$deployCmd = "$dmgrprofile/bin/wsadmin.$shellExt  -lang jython -user $wasUser -password $wasPwd -f $homePath/bin/was-autodeploy.py" if ($standalone eq 0 and -d $dmgrprofile);
